-- Deal images (product photos / post photos), OSM identity for seeded businesses, and image-aware feed RPCs.
alter table public.deals add column if not exists image_url text;
alter table public.businesses add column if not exists osm_id text unique;
alter table public.businesses add column if not exists brand text;
create index if not exists businesses_chain on public.businesses (chain_key) where chain_key is not null;
...
returns table (
  ...
  unit_price numeric, distance_m double precision, ends_at timestamptz, deal_count bigint, image_url text
)
language sql stable security definer set search_path = public, extensions as $$
  with origin as (select st_setsrid(st_makepoint(p_lng, p_lat), 4326)::geography as g),
  live as (
    select d.id as deal_id, d.business_id, b.name as business_name, b.slug as business_slug, b.logo_url as business_logo_url, d.title, d.price, d.quantity, d.unit, d.unit_price, d.ends_at, d.image_url,
  ...
  best as (
    select distinct on (l.slug) l.slug, l.deal_id, l.business_id, l.business_name, l.business_slug, l.business_logo_url, l.title, l.price, l.quantity, l.unit, l.unit_price, l.distance_m, l.ends_at, l.image_url
    from live l order by l.slug, l.unit_price asc, l.distance_m asc
  )
  select ci.slug, ci.display_name, ci.category, ci.comparable_unit, x.deal_id, x.business_id, x.business_name, x.business_slug, x.business_logo_url, x.title, x.price, x.quantity, x.unit, x.unit_price, x.distance_m, x.ends_at, c.n, x.image_url
...
-- Store-by-store price grid for a set of items (the "compare stores" page).
create or replace function public.store_prices(p_lat double precision, p_lng double precision, p_radius_m integer, p_slugs text[])
returns table (business_id uuid, business_name text, business_slug text, business_logo_url text, distance_m double precision, canonical_item_slug text, unit_price numeric, price numeric, quantity numeric, unit public.unit_kind, title text, deal_id uuid)
language sql stable security definer set search_path = public, extensions as $$
  with origin as (select st_setsrid(st_makepoint(p_lng, p_lat), 4326)::geography as g)
  select distinct on (b.id, d.canonical_item_slug)
    b.id, b.name, b.slug, b.logo_url, st_distance(b.location, o.g), d.canonical_item_slug, d.unit_price, d.price, d.quantity, d.unit, d.title, d.id
  from public.deals d join public.businesses b on b.id = d.business_id cross join origin o
  where d.status = 'approved' and b.is_active and b.category = 'grocery' and d.unit_price is not null
    and d.canonical_item_slug = any (p_slugs)
    and (d.starts_at is null or d.starts_at <= now()) and (d.ends_at is null or d.ends_at > now())
    and st_dwithin(b.location, o.g, p_radius_m)
  order by b.id, d.canonical_item_slug, d.unit_price asc
$$;
grant execute on function public.store_prices(double precision, double precision, integer, text[]) to anon, authenticated;

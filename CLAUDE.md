...
- `pnpm google:sync [--dry] [--limit N] [--force] [--photos N] [--only-missing]` — Google Places (New) enrichment for every active business: rating, review count, price level, cuisines (`src/lib/places/cuisines.ts`), hours (`businesses.hours`, Google periods), up to N photos mirrored to `media/places/<id>/`, 5 reviews into `business_reviews`. Needs `GOOGLE_PLACES_API_KEY` (Places API (New) enabled; ~$0.03 per place; re-run monthly to stay within Google's 30-day content policy). Matches are verified by name similarity + distance < 250 m.
- `pnpm chains:sync [--dry] [--only k1,k2]` — attach one fan-out offers page per chain from `src/lib/chains/index.ts` (sets `chain_key` on the brand's stores, `sources.fan_out=true` on the anchor store) and queue the crawl; extracted deals are copied to every store of the chain.
